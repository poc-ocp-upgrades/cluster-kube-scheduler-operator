...
func createTargetConfigReconciler_v311_00_to_latest(c TargetConfigReconciler, recorder events.Recorder, operatorConfig *operatorv1.KubeScheduler) (bool, error) {
	...
	if len(errors) > 0 {
		...
		v1helpers.SetOperatorCondition(&operatorConfig.Status.Conditions, operatorv1.OperatorCondition{Type: "TargetConfigReconcilerDegraded", Status: operatorv1.ConditionTrue, Reason: "SynchronizationError", Message: message})
	...
	v1helpers.SetOperatorCondition(&operatorConfig.Status.Conditions, operatorv1.OperatorCondition{Type: "TargetConfigReconcilerDegraded", Status: operatorv1.ConditionFalse})
	if !reflect.DeepEqual(operatorConfigOriginal, operatorConfig) {
		_, updateError := c.operatorConfigClient.KubeSchedulers().UpdateStatus(operatorConfig)
		if updateError != nil {
			return true, updateError
